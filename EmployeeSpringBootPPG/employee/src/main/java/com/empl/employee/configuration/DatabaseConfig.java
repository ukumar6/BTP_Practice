package com.empl.employee.configuration;
...
public class DatabaseConfig extends AbstractCloudConfig {
        ...
        @Value("${vcap.services.postgre.credentials.username}")
        private String username;

        @Value("${vcap.services.postgre.credentials.password}")
        private String password;

        @Value("${vcap.services.postgre.credentials.hostname}")
        private String hostname;

        @Value("${vcap.services.postgre.credentials.port}")
        private String port;

        @Value("${vcap.services.postgre.credentials.dbname}")
        ...
        public DataSource dataSource() {
                ...
                cloudFoundryDataSourceConfigLogger.info("Detected Host name is : " +
                                this.hostname);
                cloudFoundryDataSourceConfigLogger.info("Detected port name is : " +
                                this.port);
                cloudFoundryDataSourceConfigLogger.info("Detected DB name is : " +
                                this.dbname);
                cloudFoundryDataSourceConfigLogger.info("Detected User name is : " +
                                this.username);
        ...
        /**
         * (Step 2b) Based on an {@link EntityManagerFactory} (provided using the
         * method above), provides a {@link JpaTransactionManager} (another core class
         * of JPA).
         */
